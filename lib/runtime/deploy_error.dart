...
/// apt 网络获取失败的真实分类文案（部署中心用户可见）
///
/// 根因：App 部署的 Ubuntu noble rootfs 出厂 apt 源为
/// `http://ports.ubuntu.com/ubuntu-ports`，部分网络环境 TCP:80
/// 连接失败 → `E: Failed to fetch ... exit=100`，与「权限」无关。
const String kAptNetworkFailure = 'APT 下载失败 / 无法连接 Ubuntu 镜像';

/// apt 网络获取失败的用户建议（已自动尝试官方 HTTPS 与国内镜像）
const String kAptNetworkSuggestion = '已自动尝试官方 HTTPS 与国内镜像源仍失败。'
    '请：① 切换 Wi-Fi ↔ 移动数据后重试 ② 配置公共 DNS（8.8.8.8）'
    ' ③ 检查系统时间是否正确（HTTPS 需要证书校验）';

/// 用户建议映射（参照 Firecrawl 每个错误类内置建议文本）
class DeployErrorSuggestions {
  static const Map<DeployErrorCode, String> suggestions = {
    DeployErrorCode.dnsResolutionFailed: 'DNS 解析失败。① 切换 Wi-Fi ↔ 移动数据后重试 '
        '② 开启飞行模式再关闭 ③ 配置公共 DNS：8.8.8.8',
    DeployErrorCode.dnsAllResolversExhausted:
        '所有 DNS 解析器均失败。请检查网络连接，或等待一段时间后重试',
    DeployErrorCode.connectionTimeout: '连接超时。① 检查网络连接 ② 稍后重试 ③ 尝试使用镜像源',
    DeployErrorCode.connectionReset: '连接被重置。① 检查网络稳定性 ② 稍后重试',
    DeployErrorCode.sslHandshakeFailed: 'SSL 握手失败。① 检查系统时间是否正确 ② 尝试使用 HTTP 镜像源',
    DeployErrorCode.httpError: '服务器返回错误状态码。请稍后重试',
    DeployErrorCode.downloadInterrupted: '下载中断。① 检查网络稳定性 ② 自动重试中...',
    DeployErrorCode.sizeMismatch: '文件大小不匹配，下载可能不完整。将自动重试',
    DeployErrorCode.sha256Mismatch: '文件校验失败，下载文件可能已损坏。将自动切换下载源重试',
    ...
    DeployErrorCode.extractionFailed: '解压失败。可能的原因是下载文件损坏，将自动重新下载',
    DeployErrorCode.permissionDenied: '权限不足。请确保：① 已授予存储权限 ② 重启应用后重试 ③ 清理缓存目录',
    DeployErrorCode.diskFull: '存储空间不足。请释放空间后重试，至少需要 500MB 可用空间',
    DeployErrorCode.binaryCorrupted: '下载的二进制文件已损坏。将自动重新下载',
    DeployErrorCode.healthCheckFailed: '安装后验证失败。请尝试重新安装',
    DeployErrorCode.archNotSupported:
        '当前设备架构暂不支持。该 Runtime 仅支持 arm64-v8a / aarch64',
    DeployErrorCode.dependencyMissing: '缺少依赖。请先安装所需依赖后重试',
    DeployErrorCode.toolInstallationFailed:
        '系统工具安装失败。请确保 Linux Runtime 已初始化且网络连接正常',
    DeployErrorCode.versionMismatch: '版本不匹配。将尝试安装兼容版本',
    DeployErrorCode.aptUpdateFailed:
        '$kAptNetworkFailure。$kAptNetworkSuggestion',
    DeployErrorCode.aptInstallFailed:
        '$kAptNetworkFailure。$kAptNetworkSuggestion',
    DeployErrorCode.deploymentInProgress: '已有部署任务正在进行中。请等待当前任务完成后重试',
    DeployErrorCode.unknown: '未知错误。请查看日志了解更多信息',
  };
