...
**标题：codex-mobile-pro：一个想第一时间把 DSH 跑进手机的 Agent Harness 开源项目**
...
**我做的项目一句话介绍：**
> codex-mobile-pro 是一个 **MIT 开源、跑在手机上的 Agent Harness**——在 Termux 里一键部署 Codex / DeepSeek CLI Agent，配一个自己写的 Flutter App（内置 ANSI/PTY 终端、AI 对话、代码编辑、文件管理），自带 **70 个预装 skills**（代码审查、TDD、调试、研究、逆向、多模态……开箱即用）。
...
3. **补移动端空白**：DSH 现在主要跑在桌面 Web UI + npm 上，还没有原生移动端。codex-mobile-pro 有完整的 **Flutter + PTY/ANSI 终端渲染**经验（手机真机上跑通 Codex、中文交互、国内直连），如果 DSH 想覆盖手机场景，这块我可以直接帮忙。

**项目现状（不吹，说实话）：**
- 2026-07-27 建仓，**每天在提交、活跃维护**，最近一次推送就是昨天
- 本地真实自用跑通：手机 Termux 上跑 Codex / DeepSeek，不是概念项目
...
2. 我很愿意在 DSH 稳定后**第一时间接入**，或直接承接**移动端适配 / 插件迁移**这类共建工作，需要我怎么配合都行。
...
崔老师好，我是 Agent Harness 开源项目 codex-mobile-pro 的作者（GitHub: malaxiya2019），MIT 开源、每天维护、70 个 skills、手机真机跑通 Codex/DeepSeek。从内测招募跟到 8/13 开源，特别想参与 DSH 生态——能给个 API 额度或接入支持吗？我也愿意第一时间把 DSH 接进移动端场景。仓库：github.com/malaxiya2019/codex-mobile-pro
...
3. **GitHub Discussions**：发一帖，标题《codex-mobile-pro：想为 DSH 做移动端接入的 Agent Harness 项目》，正文贴「版本 A」全文，结尾补一句"很愿意参与插件生态共建"。
